...
function Expand-ProcessingStateNode {
    param(
        [AllowNull()]
        [object]$Node
    )
    ...
    if ($Node -is [System.Array]) {
        foreach ($child in $Node) {
            Expand-ProcessingStateNode -Node $child
        }
    ...
    if (
        $propertyNames -contains "record_key" -and
        $propertyNames -contains "current_state"
    ) {
        Write-Output $Node
        return
    }

    if ($propertyNames -contains "records") {
        Expand-ProcessingStateNode -Node $Node.records
        return
    }
}

$parsedState = $stateContent | ConvertFrom-Json

$script:stateRecords = @(
    Expand-ProcessingStateNode -Node $parsedState
)

if ($script:stateRecords.Count -eq 0) {
    throw (
        "No valid processing records were found in: $stateFile. " +
        "Restore the last known-good Step 24 backup before continuing."
    )
}

Write-Host (
    "Loaded processing records: " +
    $script:stateRecords.Count
...
function Save-StateRecords {
    ...
    # Windows PowerShell 5.1 can collapse a one-record JSON array during
    # round trips. Serialize each record separately and explicitly wrap the
    # output in a single top-level JSON array.
    $serializedRecords = @(
    ...
    $json = if ($serializedRecords.Count -eq 0) {
        "[]"
    }
    else {
        "[`r`n" +
        ($serializedRecords -join ",`r`n") +
        "`r`n]"
    }
    ...
    # Validate the temporary JSON before replacing the live state file.
    $validationContent = Get-Content `
        -Path $temporaryFile `
        -Raw

    $validationParsed = $validationContent | ConvertFrom-Json
    $validationRecords = @(
        Expand-ProcessingStateNode -Node $validationParsed
    )

    if (
        $validationRecords.Count -ne
        @($script:stateRecords).Count
    ) {
        Remove-Item $temporaryFile -Force -ErrorAction SilentlyContinue
        throw (
            "Refusing to replace the state file because JSON validation " +
            "returned $($validationRecords.Count) record(s), expected " +
            "$(@($script:stateRecords).Count)."
        )
    }
...
function Set-ObjectProperty {
    ...
    # ConvertFrom-Json returns PSCustomObject records. Add-Member -Force
    # safely creates a missing NoteProperty or replaces its current value.
    # This avoids false negatives from direct Properties[...] lookups in
    # Windows PowerShell 5.1 when a property already exists with a null value.
    Add-Member `
        -InputObject $Object `
...
function ConvertTo-StateString {
    ...
    # A state should be scalar, but older/malformed JSON may deserialize it
    # as a one-element collection or wrapped object. Normalize it before
    # passing it to typed function parameters or saving it to history.
...
function Add-HistoryEvent {
    ...
    $normalizedFromState = ConvertTo-StateString -Value $FromState
    $normalizedToState = ConvertTo-StateString -Value $ToState
    $normalizedErrorCode = ConvertTo-StateString -Value $ErrorCode
    $normalizedErrorMessage = ConvertTo-StateString -Value $ErrorMessage

    if ([string]::IsNullOrWhiteSpace($normalizedToState)) {
        throw "History event requires a non-empty ToState value."
    }
    ...
    if (
        $recordPropertyNames -contains "history" -and
...
# ------------------------------------------------------------
# Add only missing Step 25 fields. Existing Step 24 values such as
# subject, current_state, raw evidence paths, and attachment data are never
# overwritten by migration defaults.

$processingStateSchemaVersion = "1.2"
$migrationTimestamp = (
    Get-Date
).ToUniversalTime().ToString("o")
...
foreach ($stateRecord in @($script:stateRecords)) {
    ...
    $propertyNames = @(
        $stateRecord.PSObject.Properties |
        ForEach-Object { $_.Name }
    )
    ...
    foreach ($requiredName in @(
        ...
        if ($propertyNames -notcontains $requiredName) {
            throw (
                "Processing record is missing required property " +
                "'$requiredName'. Restore the Step 24 state backup."
            )
        }
    }

    $normalizedCurrentState = ConvertTo-StateString `
        -Value $stateRecord.current_state

    if ([string]::IsNullOrWhiteSpace($normalizedCurrentState)) {
        throw (
            "Processing record '$($stateRecord.record_key)' has an empty " +
            "current_state. Restore the Step 24 state backup."
        )
    }

    if (
        $stateRecord.current_state -isnot [string] -or
        [string]$stateRecord.current_state -ne
            $normalizedCurrentState
    ) {
        Set-ObjectProperty `
            -Object $stateRecord `
            -Name "current_state" `
            -Value $normalizedCurrentState
    ...
    if ($null -eq $stateRecord.history) {
        Set-ObjectProperty `
            -Object $stateRecord `
            -Name "history" `
            -Value @()
...
Write-Host ""
Write-Host "Loaded state summary:" -ForegroundColor Cyan
$script:stateRecords |
    Select-Object subject,current_state,previous_state |
...
function Assert-ClassificationValid {
    ...
    $classificationPropertyNames = @(
        $Classification.PSObject.Properties |
        ForEach-Object { $_.Name }
    )

    foreach ($property in $requiredProperties) {
        if ($classificationPropertyNames -notcontains $property) {
            throw (
...
$eligibleRecords = @(
    $script:stateRecords |
    Where-Object {
        $normalizedState = ConvertTo-StateString `
            -Value $_.current_state

        (
            $normalizedState -eq "ATTACHMENTS_SAVED"
        ) -or (
            $Reclassify -and
            $normalizedState -eq "CLASSIFIED"
        )
    }
)
...
foreach ($record in $eligibleRecords) {
    ...
    try {
        ...
        $oldState = ConvertTo-StateString -Value $record.current_state
