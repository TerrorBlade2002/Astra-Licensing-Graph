...
function Add-HistoryEvent {
    ...
    if (
        @($Record.PSObject.Properties.Match("history")).Count -gt 0 -and
...
foreach ($stateRecord in $script:stateRecords) {
    ...
    # Normalize state fields that may have been serialized as arrays or
    # non-string values by an earlier script version.
    if (@($stateRecord.PSObject.Properties.Match("current_state")).Count -gt 0) {
        $normalizedCurrentState = ConvertTo-StateString `
            -Value $stateRecord.current_state

        if ($stateRecord.current_state -isnot [string] -or
            [string]$stateRecord.current_state -ne $normalizedCurrentState) {
            Set-ObjectProperty `
                -Object $stateRecord `
                -Name "current_state" `
    ...
    if (@($stateRecord.PSObject.Properties.Match("previous_state")).Count -gt 0) {
        $normalizedPreviousState = ConvertTo-StateString `
            -Value $stateRecord.previous_state

        if ($null -ne $stateRecord.previous_state -and
            ($stateRecord.previous_state -isnot [string] -or
             [string]$stateRecord.previous_state -ne $normalizedPreviousState)) {
            Set-ObjectProperty `
                -Object $stateRecord `
                -Name "previous_state" `
    ...
    foreach ($propertyDefinition in $statePropertyDefaults.GetEnumerator()) {
        ...
        if (@($stateRecord.PSObject.Properties.Match($propertyName)).Count -eq 0) {
            Set-ObjectProperty `
                -Object $stateRecord `
                -Name $propertyName `
                -Value $defaultValue
...
function Assert-ClassificationValid {
    ...
    foreach ($property in $requiredProperties) {
        if (
            @($Classification.PSObject.Properties.Match($property)).Count -eq 0
        ) {
            throw (
